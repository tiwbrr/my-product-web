...
export function ProductForm({ product, compact = false }: { product?: Product; compact?: boolean }) {
  ...
  return (
    <form ref={formRef} action={formAction} className={`product-form ${compact ? "product-form-compact" : ""}`}>
      ...
      <div className="field-row"><label>ชื่อสินค้า<input name="name" defaultValue={product?.name} placeholder="เช่น กระเป๋า Canvas Weekend" required /></label><label>หมวดหมู่<input name="category" defaultValue={product?.category} placeholder="เช่น เสื้อผ้า" required /></label></div>
      <label>รายละเอียดสินค้า<textarea name="description" defaultValue={product?.description} placeholder="บอกจุดเด่น วัสดุ ขนาด หรือวิธีใช้งาน" rows={compact ? 3 : 4} required minLength={10} /></label>
      ...
      <button className="button button-dark button-wide" disabled={pending}>{pending ? "กำลังบันทึก..." : product ? "บันทึกการแก้ไข" : "+ เพิ่มสินค้าเข้าร้าน"}</button>
    </form>
  );
}
