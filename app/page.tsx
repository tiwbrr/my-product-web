...
import { getChatMessages, getGameCategories, getProducts, getStoreSettings, getYouTubeQueue } from "@/lib/store";
...
export default async function Home({ searchParams }: { searchParams: Promise<CatalogSearchParams> }) {
  ...
  const [products, settings, categories, messages, youtubeQueue] = await Promise.all([
    getProducts(),
    getStoreSettings(),
    getGameCategories(),
  ...
  return <main className="sell-site">
    ...
    <YouTubePlaylist url={settings.youtubePlaylistUrl} user={user} initialQueue={youtubeQueue} />
